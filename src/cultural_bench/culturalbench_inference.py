...
async def make_api_call_to_gpt(prompt, model) -> List:
...
async def limited_api_call(semaphore, prompt, answer, country, q_id, model) -> List:
...
async def main(model, language, include_country):
    ...
    MODEL_NAME = model # "Meta-Llama-3-1-70B-Instruct-htzs", 'GPT-4o'
    data_dir = f'data/cultural_bench'
    ...
    results = await tqdm_asyncio.gather(*tasks, total=len(tasks), desc=f"Processing_{LANGUAGE}_{INCLUDE_COUNTRY}_{model}")
...
if __name__ == "__main__":
    ...
    # weave.init(f"culturalbench_inference_{args.lang}_{args.include_country}_{args.model}")
